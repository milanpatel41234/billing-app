...
import { contactApi, noteApi, productApi, taxApi } from "../../assets/apis";
...
const reasonSelectOptions = [
    ...
    { label: 'Changing in POS', value: 'Changing in POS' },
...
const ManageNotes = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(noteApi, payload);
    ...
    try {
      if (invoice.length === 0) {
        const response = await axios.get(
          `${noteApi}/check_inv?inv=${invoiceData.invoice}`
        );
  ...
  useEffect(() => {
    const fetchSrnNo = async () => {
      try {
        const response = await axios.get(noteApi + "/get_srn");
        if (response.data.success) {
          setInvoiceData({...invoiceData , invoice: response.data.srn});
        }
    ...
    const fetchInvNo = async () => {
      try {
        const respon = await axios.get(noteApi + "/get_inv");
        if (respon.data.success) {
          setInvoiceNo(respon.data.salesInvoices.map((i)=> ({label:i, value:i})));
        }
        
      } catch (error) {}
    };
    if (InvoiceRedux.NoteData) {
      setInvoice(JSON.parse(InvoiceRedux.NoteData.all_products));
      setCheckedCharges(JSON.parse(InvoiceRedux.NoteData.all_checks));
      setOtherInfo(JSON.parse(InvoiceRedux.NoteData.other_info));
      setOtherCharges(JSON.parse(InvoiceRedux.NoteData.other_charges));
      setInvoiceData({
        customer: {
          label: InvoiceRedux.NoteData.name,
          value: InvoiceRedux.NoteData.name,
        },
        type: {
          label: InvoiceRedux.NoteData.type,
          value: InvoiceRedux.NoteData.type,
        },
        invoice: InvoiceRedux.NoteData.invoice,
        date: InvoiceRedux.NoteData.date,
      });
      setEditInvoiceState(InvoiceRedux.NoteData.id);
      dispatch(InvoiceActions.editNote(null));
    } else {
      fetchInvNo();
      //fetchSrnNo()
    }
  ...
  useEffect(() => {
    ...
    invoice.forEach((item) => {
      ...
      tempAmount1 = Math.floor(tempAmount1)
      let tempGstPerc =
        item.tax?.label?.toUpperCase().includes('GST')
          ? +item.tax?.rate
          : +0;
      let tempDiscount =
          ...
          <div className="col-sm-12">
            <div className="d-flex justify-content-between m-1 p-1 align-items-baseline ">
              <h3 className="ukhd mb-3">
                {" "}
                {editInvoiceState ? "Edit" : "Add"} Debit/Credit Note{" "}
              </h3>
              <button
                type="button"
                    ...
                    >
                        ...
                        <div className="form-group">
                          <div className="select-container ">
                            <label className="col-12 ">Document Type</label>
                            <CreatableSelect
                              // styles={customStyles}
                              name="documentType"
                              className="w-100"
                              ...
                              onChange={(val) => updateInvoiceData(val, "documentType")}
                              isClearable
                              options={[
                                { label: "Debit", value: "Debit" },
                                { label: "Credite", value: "Credite" },
                              ]}
                      ...
                      <div className="col-3">
                        <div className="form-group">
                          <div className="select-container ">
                            ...
                            <CreatableSelect
                              // styles={customStyles}
                              name="effectOnInventry"
                              className="w-100"
                              value={invoiceData.effectOnInventry}
                              onChange={(val) =>
                                updateInvoiceData(val, "effectOnInventry")
                              }
                              defaultValue={{label:'No Effect', value:'No Effect'}}
                              options={[{label:'No Effect', value:'No Effect'},{label:'Decrease', value:'Decrease'}]}
                            />
                          </div>
                        </div>
                      </div>
