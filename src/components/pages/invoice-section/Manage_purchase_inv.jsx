...
const ManagePurchaseInv = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const payload = {
      name: invoiceData.suplier?.label,
    ...
    try {
      const response = await axios.post(purchaseInvoiceApi, payload);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await axios.put(
        `${purchaseInvoiceApi}/${editInvoiceState}`,
        payload
      );
    ...
    try {
      if (invoice.length === 0) {
        const response = await axios.get(
          `${purchaseInvoiceApi}/check_inv?inv=${invoiceData.invoice}`
        );
  ...
  useEffect(() => {
    const fetchInvNo = async () => {
      try {
        const response = await axios.get(purchaseInvoiceApi + "/get_inv");
        if (response.data.success) {
          setInvoiceData({ invoice: response.data.invoice, quantity: 1 });
        }
      } catch (error) {}
    };
    if (InvoiceRedux.PurchaseInvoiceData) {
      setInvoice(JSON.parse(InvoiceRedux.PurchaseInvoiceData.all_products));
      setCheckedCharges(
        JSON.parse(InvoiceRedux.PurchaseInvoiceData.all_checks)
      );
      setOtherInfo(JSON.parse(InvoiceRedux.PurchaseInvoiceData.other_info));
      setOtherCharges(
        JSON.parse(InvoiceRedux.PurchaseInvoiceData.other_charges)
      );
      setInvoiceData({
        suplier: {
          label: InvoiceRedux.PurchaseInvoiceData.name,
          value: InvoiceRedux.PurchaseInvoiceData.name,
        },
        type: {
          label: InvoiceRedux.PurchaseInvoiceData.type,
          value: InvoiceRedux.PurchaseInvoiceData.type,
        },
        invoice: InvoiceRedux.PurchaseInvoiceData.invoice,
        date: InvoiceRedux.PurchaseInvoiceData.date,
      });
      setEditInvoiceState(InvoiceRedux.PurchaseInvoiceData.id);
      dispatch(InvoiceActions.editPurchaseInvoice(null));
    } else {
      fetchInvNo();
    }
  }, []);
    ...
    try {
      ...
      setContacts(
        response.data.contacts
          .filter((i) => i.contactType === "suplier")
          .map((item) => ({
            ...item,
            label: item.name,
            value: item.name,
          ...
          <div className="col-sm-12">
            <div className="d-flex justify-content-between m-1 p-1 align-items-baseline ">
              <h3 className="ukhd mb-3">
                {" "}
                {editInvoiceState ? "Edit" : "Add"} Purchase Invoice{" "}
              </h3>
              <button
                type="button"
                className="btn btn-warning btn-sm"
                onClick={() => navigate("/purchase_invoice")}
              >
                        ...
                        <div className="form-group">
                          <div className="select-container ">
                            ...
                            <CreatableSelect
                              ...
                              name="suplier"
                              className="w-100"
                              value={invoiceData.suplier}
                              onChange={(val) =>
                                updateInvoiceData(val, "suplier")
                              }
                              isClearable
                              options={contacts}
...
export default ManagePurchaseInv;
